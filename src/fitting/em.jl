...
function mstep!(model, ps, opt, x, batchsize)
	...
	gs = @timeit to "gradient" threadedgrad(i -> -sum(batchpathlogpdf(model, xx[:,i], paths[i])), ps, size(xx,2))
	# gs = @timeit to "gradient" gradient(() -> -sum(batchpathlogpdf(model, xx, paths)), ps)
