...
_explicitgrad(::typeof(identity), x) = 1f0
_explicitgrad(::typeof(tanh), x) = 1f0 - tanh(x)^2
_explicitgrad(::typeof(NNlib.σ), x) = σ(x)*(1f0 - σ(x))
_explicitgrad(::typeof(NNlib.leakyrelu), x) = ifelse(x > 0, NNlib.oftype(x, 1), NNlib.oftype(x, 0.01))
_explicitgrad(::typeof(invleakyrelu), x) = ifelse(x > 0, NNlib.oftype(x, 1), NNlib.oftype(x,100))
function _explicitgrad(::typeof(NNlib.selu), x) 
  λ = oftype(x/1, 1.0507009873554804934193349852946)
  α = oftype(x/1, 1.6732632423543772848170429916717)
  λ * ifelse(x > 0, 1, α * exp(x))
end

function _explicitgrad(::typeof(invselu), x) 
  λ = oftype(x/1, 1.0507009873554804934193349852946)
  α = oftype(x/1, 1.6732632423543772848170429916717)
  ifelse(x > 0, 1 / λ, 1 / (λ * α * (1 + x/(λ * α) + eps(x))))
end

function _explicitgrad(::typeof(invtanh), x) 
  x > 0.9999f0 && return(0)
  x < -0.9999f0 && return(0)
  (1/(1 + x) + 1/(1 - x)) / 2
end
