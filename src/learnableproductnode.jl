...
function treelogpdf(p::LearnableProductNode, x, path)
	o = treelogpdf(p.child, x, path[1][1])
	for i in 2:length(path)
		o += treelogpdf(p.child, x, path[i][1])
	end
	o
end
...
function _maptree(m::LearnableProductNode, x, s::AbstractScope = NoScope())
	@error "_maptree(m::LearnableProductNode, x, s::AbstractScope) is fundamentally broken"
end
