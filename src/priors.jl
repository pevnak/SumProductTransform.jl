...
function maptree(m, x, s::AbstractScope)
	n = nobs(x)
	maptree(m, x, s, collect(Iterators.partition(1:n, div(n,min(n,Threads.nthreads())))))
end

function maptree(m, x, s::AbstractScope, segments::Vector)
    if length(segments) == 1
        return(_maptree(m, getobs(x, segments[1]), s))
    else 
        i = div(length(segments),2)
        s1, s2 = segments[1:i], segments[i+1:end]
        ref1 = Threads.@spawn maptree(m, x, s, s1)
        ref2 = Threads.@spawn maptree(m, x, s, s2)
