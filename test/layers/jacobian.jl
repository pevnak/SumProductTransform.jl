using SumProductTransform, Test, LinearAlgebra, Flux, Zygote, FiniteDifferences
using SumProductTransform: SVDDense, LUDense
...
@testset "Gradient of the likelihood SVD" begin
	...
	x = randn(2,10)
	fdm = central_fdm(5, 1)
	@test isapprox(gradient(x -> sum(lkl(m, x)), x)[1], grad(fdm, x -> sum(lkl(m, x)), x)[1], rtol = 5e-4)
	m = Chain(SVDDense(2,selu), SVDDense(2,selu), SVDDense(2,identity))
	@test isapprox(gradient(x -> sum(lkl(m, x)), x)[1], grad(fdm, x -> sum(lkl(m, x)), x)[1], rtol = 5e-4)
end
